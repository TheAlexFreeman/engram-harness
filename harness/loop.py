...
from typing import Any, cast  # noqa: F401  (cast retained for downstream readability)

from harness.compaction import maybe_compact, maybe_full_compact

# Imports below are re-exported re-imports from the loop_guards module
# extracted in P2.1.2; their original private aliases are kept so any
# external module that imported them from loop.py keeps working.
from harness.loop_guards import (  # noqa: F401  (re-exported for back-compat)
    DEFAULT_TOOL_PATTERN_GUARD_MESSAGE as _DEFAULT_TOOL_PATTERN_GUARD_MESSAGE,
)
from harness.loop_guards import (  # noqa: F401
    MUTATING_FILE_TOOLS as _MUTATING_FILE_TOOLS,
)
from harness.loop_guards import (  # noqa: F401
    SMALL_READ_LIMIT_CHARS as _SMALL_READ_LIMIT_CHARS,
)
from harness.loop_guards import (  # noqa: F401
    SMALL_READ_MAX_LINES as _SMALL_READ_MAX_LINES,
)
from harness.loop_guards import (  # noqa: F401
    ReadFilePatternEvent as _ReadFilePatternEvent,
)
from harness.loop_guards import (  # noqa: F401
    ToolPatternDiagnostic as _ToolPatternDiagnostic,
)
from harness.loop_guards import (
    ToolPatternGuardState as _ToolPatternGuardState,
)
from harness.loop_guards import (  # noqa: F401
    hash_result as _hash_result,
)
from harness.loop_guards import (  # noqa: F401
    normalize_tool_path as _normalize_tool_path,
)
from harness.loop_guards import (  # noqa: F401
...
from harness.loop_guards import (
    signature_preview as _signature_preview,
)
from harness.loop_guards import (
    tool_batch_signature as _tool_batch_signature,
)
...
# ---------------------------------------------------------------------------
# Loop messaging constants — kept in loop.py because they're tightly bound
# to the run_until_idle implementation. The pattern-guard constants and
# state machine moved to harness.loop_guards (P2.1.2).
# ---------------------------------------------------------------------------
...
def run_until_idle(
    ...
    *,
    ...
    for turn in range(max_turns):
        ...
        # B2 Layer 2: high-water-mark compaction. Runs after tool_results are
        # appended and after the pause exit, before any nudge logic — that way
        # we never compact a freshly-injected nudge nor compact past a paused
        # boundary. The cost of the (rare) compaction model call is folded
        # into the session total so accounting stays honest. Failures degrade
        # gracefully; the loop must never fail because compaction failed.
        compaction_result = maybe_compact(
            messages,
            mode,
            tracer,
            input_tokens=turn_usage.input_tokens,
            threshold_tokens=compaction_input_token_threshold,
            pricing=pricing,
        )
