...
5. **`memory: trace` event taxonomy is buried.** The common event labels
   ...
   and the section doesn't convey when tracing is _required_ vs. optional.
...
3. In `harness/cli.py` and `harness/loop.py`, audit all call-sites to
...
4. Document the three meaningful combinations in a docstring:
...
3. `pyproject.toml` includes `harness/prompts/*.md` as package data.

4. Keep `system_prompt_native()` and `system_prompt_text()` as the public
