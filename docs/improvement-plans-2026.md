...
**Proposed shape (as shipped — see below for the original
path-aware sketch).**

1. Each role declares which *categories* of tool it denies, derived
  from each tool's name + `mutates` attribute. A tool denied by
   the active role is removed from the registry entirely — the model
   never sees it, so it can't try to call it. Removing > rejecting:
   the model wastes no turns on tools it can't use, and the role
   text in the system prompt (F1) explains the policy.
2. The boundary is computed once at session start, in
  `apply_role_denials` called from `build_tools`, so the
   effective tool set is stable for the session and inspectable.
3. Categorization is auto-derived (`categorize_tool`), so a fresh
  `edit_file2` with `mutates=True` is caught by the
   `code_writes` rule without anyone updating role config.
4. Per-call trace event `role_write_denied` was *not* added in F2
  v1 — there are no per-call denials to record (the tool isn't in
   the registry to be called). Session-start visibility is via a
   stderr log of the active role and the tool names removed. F4
   will surface this in the trace records and `harness status`.
5. Subagent inheritance handled in F3 — F2 only applies to the
  parent session.

**Original sketch (path-aware, deferred).** The first draft proposed
a runtime guard inside `Edit` / `Write` that consults the active
role and rejects writes outside `<harness_project_root>/workspace/`.
That preserves Edit/Write's tool surface for `plan`/`research`
sessions (they could still write to scratch under `workspace/`) but
introduces a more complex "this tool exists but rejects on bad
paths" semantic. Tool-removal is simpler, cleaner for the model, and
sufficient for the role definitions as written — `plan`/`research`
have `work_`* for workspace writes and don't need raw Edit/Write.
Path-aware guarding can come back as a v2 if the simple design
proves too coarse.

**Files (as shipped):** new `harness/safety/role_guard.py`
(`categorize_tool`, `apply_role_denials`,
`ROLE_DENIED_CATEGORIES` table); `harness/tool_registry.py`
(threads `role` through `build_tools`); `harness/cli.py`
(forwards `role` from `SessionConfig`);
`harness/tests/test_role_guard.py` (42 tests covering each role,
category derivation, profile + role composition, and the chat
...
**Risks:** the categorization layer is name- and `mutates`-driven,
which depends on tool authors following conventions. The
`test_categorize_tool_by_name` parametric test pins down the
classification for every tool currently in the FULL registry, so a
tool that violates the convention would fail the test rather than
silently slip past the role guard. Tool-removal also means the model
has no "structured rejection" feedback loop — it just doesn't see
the tool, which is fine for the four current roles but might matter
for future roles that benefit from "you can't do X here, try Y."
